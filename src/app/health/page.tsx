...
export default function HealthContractsPage() {
  const [contracts, setContracts] = useState<HealthContract[]>([]);
      ...
      <div className="p-4 lg:p-6">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            <h1 className="text-2xl font-bold text-white">Quản lý Hợp đồng Sức khỏe</h1>
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              {currentUser?.role === 'user' && (
                <button
                  onClick={() => router.push('/health/new')}
                  className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-medium py-3 px-6 rounded-xl transition-all duration-200 w-full sm:w-auto"
                >
                  Tạo hợp đồng mới
                </button>
              )}
          ...
          <div className="mb-4">
            <input
              type="text"
              placeholder="Tìm kiếm theo mã HĐ, tên người mua, người được BH..."
              value={search}
              onChange={(e) => {
                ...
                <div className="p-2 space-y-1">
                  {statuses.map(status => (
                    <label key={status} className="flex items-center gap-2 px-3 py-2 hover:bg-white/5 rounded-lg cursor-pointer transition-colors">
                      ...
                      />
                      <span className={`text-sm ${getHealthStatusColor(status)}`}>{getHealthStatusText(status)}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2">
              ...
              {statusFilter.map(status => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all hover:opacity-80 ${getHealthStatusColor(status)}`}
                >
                  <span>{getHealthStatusText(status)}</span>
          ...
          {contracts.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-lg font-medium">Chưa có hợp đồng sức khỏe nào</p>
              {currentUser?.role === 'user' && (
                <>
                  <p className="mt-2 text-sm">Bắt đầu bằng cách tạo hợp đồng bảo hiểm sức khỏe mới</p>
                  <button
                    onClick={() => router.push('/health/new')}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-left py-3 px-4 text-white font-medium">Gói BH</th>
                      <th className="text-left py-3 px-4 text-white font-medium">Người mua</th>
                      <th className="text-left py-3 px-4 text-white font-medium">Người được BH</th>
                  ...
                  <tbody>
                    {contracts.map((contract) => (
                      <tr key={contract._id} className="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td className="py-3 px-4 text-gray-300">{contract.packageName}</td>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => router.push(`/health/${contract._id}`)}
                ...
                {contracts.map((contract) => (
                  <div key={contract._id} className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                      <div>
                        <span className="text-gray-400 block">Gói BH</span>
                        <span className="text-white">{contract.packageName}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        <span className="text-gray-400 block">Người được BH</span>
                        <span className="text-white">{contract.insuredPerson?.fullname || '-'}</span>
                      </div>
                      <div>
